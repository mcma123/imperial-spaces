...
- `server/pages/res/user-crypto.js` must accept every valid Space Agent username used by `L2/<username>`, including email-style names with `@`, because login bootstrap and session-cache keys are derived from the authenticated username
