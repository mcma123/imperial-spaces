...
- Convex-backed signup uses `/api/signup` to create the Convex verifier and matching local user profile; hosted servers need `CONVEX_URL` and `SPACE_CONVEX_AUTH_SECRET` in their own runtime environment.
