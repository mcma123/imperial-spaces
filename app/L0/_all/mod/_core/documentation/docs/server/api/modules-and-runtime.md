...
- `signup` creates a Convex-backed password account and matching local `L2/<username>` profile; the server must have `CONVEX_URL` and `SPACE_CONVEX_AUTH_SECRET` configured, and plaintext passwords are never stored
