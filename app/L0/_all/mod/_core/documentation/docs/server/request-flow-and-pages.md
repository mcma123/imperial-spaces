...
- the shared page titles are `Space Agent`, `Admin Mode | Space Agent`, `Login | Imperial Space`, `Enter Space | Space Agent`, and `Shared Space | Space Agent`
...
- page shells can declare `SPACE_PROJECT_VERSION` for server-side version injection; this deployment-branded `/login` shell keeps only that centered version label in the footer
- this deployment-branded `/login` shell uses the intro heading `Imperial Space` and removes the older three-line marketing support copy under that heading
- this deployment-branded `/login` shell also removes the public run-it-yourself CTA and the footer social-link row
