...
- hosted deployments must set `CONVEX_URL` and `SPACE_CONVEX_AUTH_SECRET` on the Space Agent server process itself; setting `SPACE_CONVEX_AUTH_SECRET` only inside the Convex deployment lets Convex validate server calls but does not configure the server to make those calls
